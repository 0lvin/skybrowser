...
addCookies(gchar ** saved_cookies, const gchar * cookies)
{
   ...
   if (posend != NULL) {
      ...
      new_cookies =
	 calloc(strlen(*saved_cookies) + posend - cookies + 2, sizeof(gchar));
	 ...
	 gpointer data, gboolean redirect_save, char *gotocharp)
{
   ...
   if (!strncmp(realurl, "http:", strlen("http:"))) {
	 ...
	 if (!strcmp(method, "POST")) {
	    ...
	    char *tmpstr =
	       calloc(strlen(realurl) + strlen(encoding) + 2, sizeof(char));
...
#ifdef banner
	    if (strstr(tmpstr, "banner") || strstr(tmpstr, "/ads.")
		|| strstr(tmpstr, "megaban.com.ua")) {
   ...
   gtk_html_stream_write(stream, msg->response_body->data,
	...
	gboolean redirect_save)
{
   ...
   if (!strcmp(method, "GET") || !strcmp(method, "POST")) {
	 ...
	 realurl =
	    (char *) calloc(strlen(action) + strlen(gtk_html_get_base(html)) +
			    2, sizeof(char));
...
on_link_clicked(GtkHTML * html, const gchar * url, gpointer data)
{
   ...
   if (gtk_html_get_base(html))
      if (!strncmp
	  (url, gtk_html_get_base(html), strlen(gtk_html_get_base(html))))
	 if (*(url + strlen(gtk_html_get_base(html))) == '#') {
	    change_position(html, url + strlen(gtk_html_get_base(html)) + 1,
			    data);
	    return;
	 }
   {
...
main(gint argc, gchar * args[])
{
   ...
   g_signal_connect(variable->html, "submit", G_CALLBACK(on_submit),
		    variable);
   g_signal_connect(variable->app, "delete-event", G_CALLBACK(on_exit_window),
		    variable);
